...
ExcludeList = ExcludeList * "BG, " #ok
ExcludeList = ExcludeList * "CY, " #LoadError: BoundsError: attempt to access 0-element Vector{Tuple{Float64, Float64}} at index [1:1]
ExcludeList = ExcludeList * "CZ, " #Infeasible message 
ExcludeList = ExcludeList * "DE, " # ok
ExcludeList = ExcludeList * "DK, " # ok
ExcludeList = ExcludeList * "EE, " #ok
ExcludeList = ExcludeList * "ES, " #LoadError: ArgumentError: reducing over an empty collection is not allowed; consider supplying `init` to the reducer
ExcludeList = ExcludeList * "FI, " # LoadError: ArgumentError: reducing over an empty collection is not allowed; consider supplying `init` to the reducer
ExcludeList = ExcludeList * "FR, " #OK
ExcludeList = ExcludeList * "GR, " #LoadError: KeyError: key :gdp_percentage not found
ExcludeList = ExcludeList * "HR, "#LoadError: KeyError: key :gdp_percentage not found
ExcludeList = ExcludeList * "HU, "#Infeasible
ExcludeList = ExcludeList * "IE, " #ok
...
ExcludeList = ExcludeList * "MT, " #LoadError: BoundsError: attempt to access 0-element Vector{Tuple{Float64, Float64}} at index [1:1]
ExcludeList = ExcludeList * "NL, " #Infeasible
ExcludeList = ExcludeList * "PL, " #Infeasible
ExcludeList = ExcludeList * "PT, " #ok
ExcludeList = ExcludeList * "RO, " # ok
ExcludeList = ExcludeList * "SE, " #ok
ExcludeList = ExcludeList * "SI, " #Infeasible
